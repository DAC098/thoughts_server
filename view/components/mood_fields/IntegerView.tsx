...
import { Integer, MoodFieldTypeName } from "../../api/mood_field_types"
        ...
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.minimum != null} onChange={(e,c) => {
                    onChange?.({type: MoodFieldTypeName.Integer, minimum: c ? 0 : null, maximum: optionalCloneInteger(config.maximum)});
                }}/>
                <SpinButton 
                    ...
                    onChange={(e,v) => {
                        let int = parseInt(v);
                        onChange?.({type: MoodFieldTypeName.Integer,minimum: int,maximum: optionalCloneInteger(config.maximum)});
                    }}
                />
            </Stack>
        </Stack>
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.maximum != null} onChange={(e,c) => {
                    onChange?.({type: MoodFieldTypeName.Integer, minimum: optionalCloneInteger(config.minimum), maximum: c ? 0 : null});
                }}/>
                <SpinButton
                    ...
                    onChange={(e,v) => {
                        let int = parseInt(v);
                        onChange?.({type: MoodFieldTypeName.Integer, minimum: optionalCloneInteger(config.minimum), maximum: int});
                    }}
                />
            </Stack>
        </Stack>
