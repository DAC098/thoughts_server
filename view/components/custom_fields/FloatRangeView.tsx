...
import { FloatRange, CustomFieldTypeName } from "../../api/custom_field_types";
        ...
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.minimum != null} onChange={(e,c) => {
                    onChange?.({type: CustomFieldTypeName.FloatRange, minimum: c ? 0 : null, maximum: config.maximum})
                }}/>
                <SpinButton
                    ...
                    onChange={(e,v) => {
                        ...
                        if (!isNaN(float) && (config.maximum != null ? float < config.maximum : true)) {
                            onChange?.({type: CustomFieldTypeName.FloatRange, minimum: float, maximum: config.maximum});
                        }
                    }}
                />
            </Stack>
        ...
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.maximum != null} onChange={(e,c) => {
                    onChange?.({type: CustomFieldTypeName.FloatRange, minimum: config.minimum, maximum: c ? 0 : null});
                }}/>
                <SpinButton
                    ...
                    onChange={(e,v) => {
                        ...
                        if (!isNaN(float) && (config.minimum != null ? float > config.minimum : true)) {
                            onChange?.({type: CustomFieldTypeName.FloatRange, minimum: config.minimum, maximum: float});
                        }
                    }}
                />
            </Stack>
