        ...
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.minimum != null} onChange={(e,c) => {
                    onChange?.({type: CustomFieldTypeName.Float, minimum: c ? 0 : null, maximum: config.maximum})
                }}/>
                <SpinButton
                    ...
                    onChange={(e,v) => {
                        ...
                        if (!isNaN(float) && (config.maximum != null ? float < config.maximum : true)) {
                            onChange?.({type: CustomFieldTypeName.Float, minimum: float, maximum: config.maximum});
                        }
                    }}
                />
            </Stack>
        ...
        <Stack tokens={{childrenGap: 8}}>
            ...
            <Stack horizontal verticalAlign="center" tokens={{childrenGap: 8}}>
                <Toggle checked={config.maximum != null} onChange={(e,c) => {
                    onChange?.({type: CustomFieldTypeName.Float, minimum: config.minimum, maximum: c ? 0 : null});
                }}/>
                <SpinButton
                    ...
                    onChange={(e,v) => {
                        ...
                        if (!isNaN(float) && (config.minimum != null ? float > config.minimum : true)) {
                            onChange?.({type: CustomFieldTypeName.Float, minimum: config.minimum, maximum: float});
                        }
                    }}
                />
            </Stack>
