...
import { TimeRange } from "../../api/mood_entry_types";
import { MoodFieldTypeName, TimeRange as TimeRangeField } from "../../api/mood_field_types";
                ...
                <Stack horizontal tokens={{childrenGap: 8}}>
                    <DatePicker
                        ...
                        onSelectDate={d => {
                            ...
                            onChange?.({type: MoodFieldTypeName.TimeRange, low: copy.toISOString(), high: finish_date.toISOString()})
                        }}
                    />
                    <TimeInput value={start_date} onChange={d => {
                        onChange?.({type: MoodFieldTypeName.TimeRange, low: d.toISOString(), high: finish_date.toISOString()});
                    }}/>
                    <TooltipHost content="sets the time to this current moment">
                        <DefaultButton text="Now" onClick={() => onChange?.({type: MoodFieldTypeName.TimeRange, low: (new Date()).toISOString(), high: finish_date.toISOString()})}/>
                    </TooltipHost>
                </Stack>
            </Stack>
            <Stack tokens={{childrenGap: 2}}>
                ...
                <Stack horizontal tokens={{childrenGap: 8}}>
                    <DatePicker
                        ...
                        onSelectDate={d => {
                            ...
                            copy.setMonth(d.getMonth());
                            copy.setDate(d.getDate());
                            onChange?.({type: MoodFieldTypeName.TimeRange, low: start_date.toISOString(), high: copy.toISOString()})
                        }}
                    />
                    <TimeInput value={finish_date} onChange={d => {
                        onChange?.({type: MoodFieldTypeName.TimeRange, low: start_date.toISOString(), high: d.toISOString()});
                    }}/>
                    <TooltipHost content="sets the time to this current moment">
                        <DefaultButton text="Now" onClick={() => onChange?.({type: MoodFieldTypeName.TimeRange, low: start_date.toISOString(), high: (new Date()).toISOString()})}/>
                    </TooltipHost>
                </Stack>
            </Stack>
        </Stack>
