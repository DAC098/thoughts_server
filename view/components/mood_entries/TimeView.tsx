...
import { Time } from "../../api/mood_entry_types";
import { MoodFieldTypeName, Time as TimeField } from "../../api/mood_field_types";
            ...
            <Stack horizontal tokens={{childrenGap: 8}}>
                <DatePicker
                    ...
                    onSelectDate={d => {
                        ...
                        onChange?.({type: MoodFieldTypeName.Time, value: copy.toISOString()});
                    }}
                />
                <TimeInput value={date} onChange={d => {
                    onChange?.({type: MoodFieldTypeName.Time, value: d.toISOString()});
                }}/>
                <TooltipHost content="sets the time to this current moment">
                    <DefaultButton text="Now" onClick={() => onChange?.({type: MoodFieldTypeName.Time, value: (new Date()).toISOString()})}/>
                </TooltipHost>
            </Stack>
        </Stack>
